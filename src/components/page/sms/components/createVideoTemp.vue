    ...
    <div class="content-content">
      <div class="left">
        <div class="preview">
          ...
          <div class="ov-auto">
            <div class="item pre-title" :class="selected === 1 ? 'active' : ''" @click="selected = 1">添加主题</div>
            <div class="item pre-title" :class="selected === 2 ? 'active' : ''" @click="selected = 2">添加签名</div>
            <!--一个元素是和一个上传或填写对应的必须有多个-->
            <draggable v-model="dom"
                       :options="{ group: 'people',animation:150,scroll:true,scrollSensitivity:200,draggable: '.item',}"
                       @start="drag = true" @end="drag = false">
              <div v-for="(v,k) in dom" :key="k" :is="v" :selected="selected" @del="del" :ind="k" :id="k"
                   :dom="dom"></div>
            </draggable>
      ...
      <div class="right">
        <el-form ref="form" :model="form" label-width="120px">
          ...
          <el-form-item label="彩信内容：" v-if="selected === 3" v-for="(v,k) in text" :key="k">
            <el-input  v-model="form.content+k" size="small" placeholder="请输入彩信内容，不超过500个字符" type="textarea" :rows="10"
...
<script>
  ...
  export default {
    ...
    watch: {
      //拖动的时候，dom元素的下标发生改变，删除的时候下标应该是对的上的啊
      dom(val, oldVal) {
        for (let i = 0; i < val.length; i++) {
          if (val[i] === 'vText') {
            this.text.push(val[i])
          } else if (val[i] === 'vAudio') {
            this.audio.push(val[i])
          } else if (val[i] === 'vImg') {
            this.imgs.push(val[i])
          } else if (val[i] === 'vVideo') {
            this.video.push(val[i])
          }
